...
class cnn_model:
    ...
    def Conv2D(self, name_, input_, shape_, stride_, with_CPM_initialize_=False, with_relu_=True):
        if with_CPM_initialize_:
            W = tf.get_variable(name_+'/W', initializer=tf.constant(self.params_original[name_ + '/W']))
            b = tf.get_variable(name_+'/b', initializer=tf.constant(self.params_original[name_ + '/b']))
        else:
            W = tf.get_variable(name_+'/W', shape=shape_, initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable(name_+'/b', shape=[shape_[-1]], initializer=tf.contrib.layers.xavier_initializer())
    ...
    def first_stage(self, image):    
        ...
        pool_2 = tf.nn.max_pool(conv2_2,[1,2,2,1],[1,2,2,1],padding='SAME') # ksize, stride, pool
        ...
        conv5_1 = self.Conv2D('conv5_1_CPM', conv4_7, [1, 1, 128, 512], 1, with_CPM_initialize_=opt.load_original_CPM, with_relu_=True)
        conv5_2 = self.Conv2D('conv5_2_CPM', conv5_1, [1, 1, 512, 15], 1, with_CPM_initialize_=opt.load_original_CPM, with_relu_=True)
        
        return conv5_2, conv4_7

    def stage_block(self, input_, conv4_7, stage_name):
        
        concat_stageX = tf.concat([input_, conv4_7], 3) # N * H * W * C

        Mconv1 = self.Conv2D('Mconv1'+'_'+stage_name, concat_stageX, [7, 7, 143, 128], 1, with_CPM_initialize_=False, with_relu_=True)
        ...
        Mconv7 = self.Conv2D('Mconv7'+'_'+stage_name, Mconv6, [7, 7, 128, 15], 1, with_CPM_initialize_=opt.load_original_CPM, with_relu_=True)
    ...
    def setup_model(self, image, gt):
        with tf.variable_scope("stage1"):
            stage_1_output, stage_1_output_2 = self.first_stage(image)
        with tf.variable_scope("stage2"):
            stage_2_output = self.stage_block(stage_1_output, stage_1_output_2, "stage2")
        with tf.variable_scope("stage3"):
            stage_3_output = self.stage_block(stage_2_output, stage_1_output_2, "stage3")
        ...
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=stage_1_output_deconved,labels=gt)+tf.nn.softmax_cross_entropy_with_logits(logits=stage_2_output_deconved,labels=gt)+tf.nn.softmax_cross_entropy_with_logits(logits=stage_3_output_deconved,labels=gt))
